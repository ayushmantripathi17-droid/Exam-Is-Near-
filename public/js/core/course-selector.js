...
function getCbse11Subjects(){
  // NOTE (phase 1 restructuring): originally returned full subject objects from
  // CBSE11_STREAMS[stream].subjects. CBSE11_STREAMS here is now the slim,
  // id-only version (see below) — full subject content lives in
  // courses/CBSE/Class 11/Syllabus/*.js. This function currently returns ids;
  // phase 2 (lazy loading) is expected to resolve ids against the loaded
  // Syllabus modules.
  if(cbse11Stream && CBSE11_STREAMS[cbse11Stream]) return CBSE11_STREAMS[cbse11Stream].subjectIds;
  return [];
}
...
function getCbse12Subjects(){
  if(cbse12Stream && CBSE12_STREAMS[cbse12Stream]) return CBSE12_STREAMS[cbse12Stream].subjectIds;
  return [];
}
