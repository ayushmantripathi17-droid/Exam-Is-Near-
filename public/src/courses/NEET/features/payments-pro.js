...
import { googleSignIn } from "../../../core/firebase-sync.js";
...
async function applyCouponCode(){
  ...
  try{
    const token = await AppStateShared.currentUser.getIdToken(true);
    const res = await fetch(CF_URLS.activateTrial, {
      method: "POST",
      headers: { "Content-Type": "application/json", "Authorization": "Bearer " + token },
      body: JSON.stringify({ data: { uid: AppStateShared.currentUser.uid, couponCode: code, plan: "monthly", validateOnly: true } })
    });
...
async function isProUser({ forceRefresh = false } = {}){
  ...
  try{
    const token = await AppStateShared.currentUser.getIdToken(/* forceRefresh */ forceRefresh);
    ...
    if(serverIsPro){
      CourseRuntime._proStatusCache = true;
      ...
      CourseRuntime._proStatusCache = false;
...
async function verifyProOrThrow(featureName){
  if(!AppStateShared.currentUser) throw new Error("sign_in_required");
...
async function _activateFreeCoupon(plan){
  ...
  try{
    ...
    const res = await fetch(CF_URLS.activateTrial, {
      method: "POST",
      headers: { "Content-Type": "application/json", "Authorization": "Bearer " + token },
      body: JSON.stringify({ data: { uid: AppStateShared.currentUser.uid, couponCode: _activeCoupon?.code||"", plan } })
    });
...
async function initiateProPayment(){
  if(!AppStateShared.currentUser){ showToast("🔐 Please sign in with Google first","info"); googleSignIn(); return; }
...
async function initiateTrialPayment(){
  if(!AppStateShared.currentUser){ showToast("🔐 Please sign in with Google first to start your trial","info"); googleSignIn(); return; }
  const already = await isProUser();
  if(already){ showToast("⭐ You're already on Pro!","success"); closeProModal(); return; }
  ...
  try{
    ...
    const res = await fetch(CF_URLS.activateTrial, {
      method: "POST",
      headers: { "Content-Type": "application/json", "Authorization": "Bearer " + token },
      body: JSON.stringify({ data: { uid: AppStateShared.currentUser.uid } })
    });
...
async function _launchRazorpay({ plan, amount, description, couponCode=null }){
  ...
  let orderData;
  try{
    const token = await AppStateShared.currentUser.getIdToken(/* forceRefresh */ true);
    const res = await fetch(CF_URLS.createOrder, {
      method: "POST",
      headers: { "Content-Type": "application/json", "Authorization": "Bearer " + token },
      body: JSON.stringify({ data: { email: AppStateShared.currentUser.email||"", name: AppStateShared.currentUser.displayName||"", plan, amount, couponCode } })
    });
    const json = await res.json();
  ...
  const options = {
    ...
    prefill:     { name: AppStateShared.currentUser.displayName||"", email: AppStateShared.currentUser.email||"" },
    notes:       { uid: AppStateShared.currentUser.uid, plan },
    theme:       { color: "#FFE66D" },
...
export { CF_BASE, CF_URLS, PRO_KEY, PRO_PRICE_INR, _activateFreeCoupon, _activeCoupon, _hideAdsForPro, _launchRazorpay, _loadRazorpay, _proCache, _resetCouponState, _updateProHeaderUI, _verifyAndGrantPro, applyCouponCode, closeProModal, initiateProPayment, initiateTrialPayment, initiateYearlyPayment, isProUser, openProModal, requirePro, verifyProOrThrow };

if (typeof window !== "undefined") {
  Object.assign(window, { CF_BASE, CF_URLS, PRO_KEY, PRO_PRICE_INR, _activateFreeCoupon, _activeCoupon, _hideAdsForPro, _launchRazorpay, _loadRazorpay, _proCache, _resetCouponState, _updateProHeaderUI, _verifyAndGrantPro, applyCouponCode, closeProModal, initiateProPayment, initiateTrialPayment, initiateYearlyPayment, isProUser, openProModal, requirePro, verifyProOrThrow });
}
