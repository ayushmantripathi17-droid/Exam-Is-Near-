...
import { isProUser, openProModal } from "./payments-pro.js";
import { AppStateShared } from "../../../core/app-state.js";
import { AiAssistantViewShared, _aiLoadSessions, _aiPersistCurrentSession, _aiSaveSessions, _aiSessionTitle, _aiSessions, getAIQuickPrompts, getAIWelcomeCards, renderAIMsgBubble } from "./ai-assistant-view.js";
import { showToast } from "../../../utils/helpers.js";
import { CBSE11_STREAMS, CBSE12_STREAMS, CourseSelectorShared, getSubjects } from "../../../core/course-selector.js";
import { getDaysLeft, getExamDate, getSubjectPct, getTotalHours } from "./exam-dates-actions.js";
import { render } from "../ui/render-core.js";

export const AiAssistantSetupShared = {};
// ══════════════════════════════════════════════════════════════
...
AiAssistantSetupShared.freeQuizCount = _initDailyCounter('ein_free_quiz_day');  // resets daily
  ...
  try{
    ...
    if(saved){
      ...
      if(Array.isArray(parsed) && parsed.length > 0){
        AiAssistantSetupShared.aiHistory = parsed;
        renderAIChat();
      }
    }
  }catch(e){}
...
async function saveAIChatHistory(){
  try{
    ...
    const toSave = AiAssistantSetupShared.aiHistory.slice(-AI_HISTORY_MAX);
...
async function askAI(prompt, isJson = false, systemOverride = null) {
  try {
    const pro = await isProUser();
    CourseRuntime._proStatusCache = pro;
    ...
    let resolvedUser = AppStateShared.currentUser;
    if (!resolvedUser && AppStateShared.auth) {
      try {
        resolvedUser = await new Promise((resolve) => {
          const unsub = AppStateShared.auth.onAuthStateChanged(u => { unsub(); resolve(u); });
...
async function sendAIMessage(){
  ...
  if(!msg||CourseRuntime.aiTyping) return;
  ...
  if(!AiAssistantViewShared._aiActiveSession){ const id='sess_'+Date.now(); AiAssistantViewShared._aiActiveSession=id; _aiLoadSessions(); _aiSessions.unshift({id,title:'New chat',ts:Date.now(),messages:[]}); _aiSaveSessions(); }
  CourseRuntime.aiTyping=true;
  AiAssistantSetupShared.aiHistory.push({role:"user",text:msg,ts:Date.now()});
  AiAssistantSetupShared.aiHistory.push({role:"ai",text:"⏳ Thinking…",ts:Date.now()});
  renderAIChat();
  ...
  const courseLabel = CourseSelectorShared.activeCourse==='neet'?'NEET UG aspirant':CourseSelectorShared.activeCourse==='jee'?'JEE (Mains & Advanced) aspirant':CourseSelectorShared.activeCourse==='nfsu'?'B.Sc. LL.B. student at NFSU':CourseSelectorShared.activeCourse==='nfsu1'?'B.Sc. LL.B. (Hons.) Sem I student at NFSU':CourseSelectorShared.activeCourse==='nfsu3'?'B.Sc. LL.B. Sem III student at NFSU':CourseSelectorShared.activeCourse==='cbse10'?'CBSE Class 10 student':CourseSelectorShared.activeCourse==='cbse11'?('CBSE Class 11 '+( CBSE11_STREAMS[CourseSelectorShared.cbse11Stream]?.label?.split('—')[1]?.trim()||'student') +' student'):CourseSelectorShared.activeCourse==='cbse12'?('CBSE Class 12 '+( CBSE12_STREAMS[CourseSelectorShared.cbse12Stream]?.label?.split('—')[1]?.trim()||'student') +' student'):'student';
  const courseExtra = CourseSelectorShared.activeCourse==='neet'?' Focus on Biology, Physics, Chemistry NCERT concepts.':CourseSelectorShared.activeCourse==='jee'?' Focus on Maths, Physics, Chemistry problem solving and formulas.':CourseSelectorShared.activeCourse==='nfsu'?' Focus on law, technology, and forensic science subjects.':CourseSelectorShared.activeCourse==='nfsu1'?' Focus on NFSU Sem I subjects: Legal Methods, Law of Tort and Consumer Protection Laws, Law and Literature, Fundamentals of Computer Organization & Embedded Systems, Basic Programming Concepts Using C, and Discrete Mathematics.':CourseSelectorShared.activeCourse==='nfsu3'?' Focus on NFSU Sem III subjects: Law of Crimes I (IPC), Constitutional Law I, Law of Contract I, Family Law I, Web Programming (HTML, JS, PHP, MySQL), and Operating System Concepts (Linux, memory, processes).':CourseSelectorShared.activeCourse==='cbse10'?' Focus on CBSE Class 10 curriculum — Maths, Science, English, Social Science.':CourseSelectorShared.activeCourse==='cbse11'?(' Focus on CBSE Class 11 curriculum for '+(CBSE11_STREAMS[CourseSelectorShared.cbse11Stream]?.desc||'the selected stream')+'. Give NCERT-focused answers.'):CourseSelectorShared.activeCourse==='cbse12'?(' Focus on CBSE Class 12 board exam for '+(CBSE12_STREAMS[CourseSelectorShared.cbse12Stream]?.desc||'the selected stream')+'. Give NCERT-focused answers.'):'';
  ...
  CourseRuntime.aiTyping=false;
  AiAssistantSetupShared.aiHistory[AiAssistantSetupShared.aiHistory.length-1]={role:"ai",text:reply};
  AiAssistantSetupShared.aiHistory[AiAssistantSetupShared.aiHistory.length-1].ts = Date.now();
  renderAIChat();
...
function renderAIChat(){
  const box=document.getElementById("ai-chat-box");
  if(!box) return;
  if(AiAssistantSetupShared.aiHistory.length===0){render();return;}
  // Append only the last message (for performance)
  const lastMsg = AiAssistantSetupShared.aiHistory[AiAssistantSetupShared.aiHistory.length-1];
  const idx = AiAssistantSetupShared.aiHistory.length-1;
  // Re-render all for simplicity
  box.innerHTML = AiAssistantSetupShared.aiHistory.map((m,i) => renderAIMsgBubble(m,i)).join("");
  // Update send button state
  const btn = document.getElementById("ai-send-btn");
  if(btn) btn.disabled = CourseRuntime.aiTyping;
  // Update status dots (sidebar model dot + any legacy ai-dot)
  document.querySelectorAll(".ai-dot,.ai-model-dot").forEach(d=>{ d.className=(d.classList.contains("ai-model-dot")?"ai-model-dot":"ai-dot")+(CourseRuntime.aiTyping?" thinking":""); });
  const topTitle=document.querySelector(".ai-topbar-title");
  if(topTitle && AiAssistantSetupShared.aiHistory.length>0) topTitle.textContent=_aiSessionTitle(AiAssistantSetupShared.aiHistory);
}

async function generateQuizFromAI(subject, count){
  const n = parseInt(count)||15;
  showToast("🧠 Generating "+n+"-question quiz…","info");
  const isJEE  = CourseSelectorShared.activeCourse === 'jee';
  const isNEET = CourseSelectorShared.activeCourse === 'neet';
  const isCBSE10  = CourseSelectorShared.activeCourse === 'cbse10';
  const isCBSE12  = CourseSelectorShared.activeCourse === 'cbse12';
...
export { AI_HISTORY_KEY, AI_HISTORY_MAX, FREE_AI_MSG_LIMIT, FREE_QUIZ_LIMIT, GROQ_MODEL, GROQ_PROXY_URL, _freeCounterDay, _incDailyCounter, _initDailyCounter, askAI, freeAiMsgCount, generateQuizFromAI, loadAIChatHistory, renderAIChat, saveAIChatHistory, sendAIMessage, setQuickPrompt, setWelcomePrompt };

if (typeof window !== "undefined") {
  Object.assign(window, { AI_HISTORY_KEY, AI_HISTORY_MAX, FREE_AI_MSG_LIMIT, FREE_QUIZ_LIMIT, GROQ_MODEL, GROQ_PROXY_URL, _freeCounterDay, _incDailyCounter, _initDailyCounter, askAI, freeAiMsgCount, generateQuizFromAI, loadAIChatHistory, renderAIChat, saveAIChatHistory, sendAIMessage, setQuickPrompt, setWelcomePrompt });
}
