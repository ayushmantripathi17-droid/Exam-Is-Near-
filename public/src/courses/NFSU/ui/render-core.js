import { renderFiles } from "../features/files-materials.js";
import { renderAbout } from "./render-about.js";
import { renderPomodoro } from "../features/pomodoro-full.js";
import { renderFlashcards } from "../features/flashcards.js";
import { renderQuiz } from "../features/quiz.js";
import { renderAnalytics } from "../features/analytics.js";
import { renderAI } from "../features/ai-assistant-view.js";
import { renderFolderPicker, renderProfile, renderSectionModal, renderSubjectSectionTabs } from "../features/subjects-profile.js";
import { renderNeetJee } from "../../JEE/features/neetjee-hub-renderer.js";
import { AppStateShared, state } from "../../../core/app-state.js";
import { RINGTONES, selectedRingtone, updateTimerDisplay } from "../features/clock-alarms-timer.js";
import { loadAIChatHistory } from "../features/ai-assistant-setup.js";
import { updateDriveBadge, updateSyncBadge, updateUserBadge } from "../../../core/firebase-sync.js";
import { esc, today } from "../../../utils/helpers.js";
import { CBSE11_STREAMS, CBSE12_STREAMS, CourseSelectorShared, getSubjects } from "../../../core/course-selector.js";
import { _examDateToISO, getDaysLeft, getExamDate, getStreak, getSubjectPct, getTotalHours, getTotalPct } from "../features/exam-dates-actions.js";
import { refOpenTopic, renderSubjectPyqAccordion, renderTopicRefModeSwitch, renderTopicRefPanel, topicRefKey } from "../features/topic-reference.js";
import { MOODS, MOOD_LABELS } from "../../../shared/app-constants.js";
...
function renderSync(){
  ...
  const s=statusMap[AppStateShared.syncStatus]||statusMap.offline;

  const userCard=AppStateShared.currentUser?`
    <div class="card" style="margin-bottom:16px;display:flex;align-items:center;gap:16px;padding:20px">
      <img src="${AppStateShared.currentUser.photoURL||''}" onerror="this.style.display='none'" style="width:54px;height:54px;border-radius:50%;border:3px solid #06D6A0;flex-shrink:0"/>
      <div style="flex:1;min-width:0">
        <div style="font-size:15px;font-weight:bold;color:#ccc;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(AppStateShared.currentUser.displayName||"User")}</div>
        <div style="font-size:10px;color:#333;margin-top:3px">UID: ${AppStateShared.syncUserId?.slice(0,16)||"—"}…</div>
      </div>
  ...
  return`<div class="fade-in">
    ...
    <div class="card card-glow" style="--glow-color:${s.color}22;margin-bottom:16px;text-align:center;padding:20px">
      ...
      <div style="display:flex;gap:8px;justify-content:center;margin-top:12px;flex-wrap:wrap">
        ...
        ${AppStateShared.currentUser?`<button class="btn-ghost" onclick="subscribeToFirestore().then(()=>showToast('✅ Pulled latest data!','success'))" style="font-size:12px">⬇ Pull Latest</button>`:''}
      </div>
    </div>
...
function renderSubjects(){
  ...
  if(!sub){
    if(CourseSelectorShared.activeCourse === 'cbse11' && !CourseSelectorShared.cbse11Stream){
    ...
    if(CourseSelectorShared.activeCourse === 'cbse12' && !CourseSelectorShared.cbse12Stream){
  ...
  return `<div class="fade-in">
    <div class="flex-wrap" style="margin-bottom:20px">${subBtns}${CourseSelectorShared.activeCourse==='cbse11'?`<button onclick="showCourseSelector('cbse_group','cbse11_group')" style="background:#FFE66D22;border:1px solid #FFE66D44;color:#FFE66D;border-radius:20px;padding:6px 13px;font-size:11px;cursor:pointer;font-family:inherit;transition:all 0.2s" title="Change stream">🔄 ${CBSE11_STREAMS[CourseSelectorShared.cbse11Stream]?.label?.split('—')[0]?.trim()||'Change Stream'}</button>`:''}${CourseSelectorShared.activeCourse==='cbse12'?`<button onclick="showCourseSelector('cbse_group','cbse12_group')" style="background:#FFE66D22;border:1px solid #FFE66D44;color:#FFE66D;border-radius:20px;padding:6px 13px;font-size:11px;cursor:pointer;font-family:inherit;transition:all 0.2s" title="Change stream">🔄 ${CBSE12_STREAMS[CourseSelectorShared.cbse12Stream]?.label?.split('—')[0]?.trim()||'Change Stream'}</button>`:''}</div>
...
export { _doRender, _renderResolvers, _renderTimer, render, renderAlarms, renderDashboard, renderLog, renderSubjects, renderSync };

if (typeof window !== "undefined") {
  Object.assign(window, { _doRender, _renderResolvers, _renderTimer, render, renderAlarms, renderDashboard, renderLog, renderSubjects, renderSync });
}
