// Auto-split from the original public/js/data/subjects.js
// Stream -> subject id groupings for Class 11. Full subject content lives in ./Syllabus/*.js
const CLASS_11_STREAMS = {
  "science_pcm": {
    "label": "🔬 Science — PCM (Physics, Chemistry, Maths)",
    "desc": "Physics · Chemistry · Mathematics + English Core",
    "subjectIds": [
      "c11-phy",
      "c11-chem",
      "c11-math",
      "c11-eng"
    ]
  },
  "science_pcb": {
    "label": "🧬 Science — PCB (Physics, Chemistry, Biology)",
    "desc": "Physics · Chemistry · Biology + English Core",
    "subjectIds": [
      "c11-phy",
      "c11-chem",
      "c11-bio",
      "c11-eng"
    ]
  },
  "science_pcmb": {
    "label": "🔭 Science — PCMB (All four)",
    "desc": "Physics · Chemistry · Maths · Biology + English Core",
    "subjectIds": [
      "c11-phy",
      "c11-chem",
      "c11-math",
      "c11-bio",
      "c11-eng"
    ]
  },
  "commerce": {
    "label": "💼 Commerce",
    "desc": "Accountancy · Business Studies · Economics · Maths + English Core",
    "subjectIds": [
      "c11-acc",
      "c11-bs",
      "c11-eco",
      "c11-math",
      "c11-eng"
    ]
  },
  "arts": {
    "label": "🎨 Arts / Humanities",
    "desc": "History · Political Science · Geography · Psychology · Sociology + English Core",
    "subjectIds": [
      "c11-hist",
      "c11-pol",
      "c11-geo",
      "c11-psyc",
      "c11-socio",
      "c11-eng"
    ]
  }
};


export { CLASS_11_STREAMS };

if (typeof window !== "undefined") {
  Object.assign(window, { CLASS_11_STREAMS });
}
