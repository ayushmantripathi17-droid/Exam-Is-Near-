...
import { NEET_WEIGHTAGE, getNeetColleges, getNeetRankDisplay } from "../../NEET/rank-data.js";
import { JEE_WEIGHTAGE, getJeeCollege, getJeePercentile } from "../rank-data.js";
...
function addMistake(){
  ...
  CourseRuntime.switchView('neetjee');
}
 
function deleteMistake(id){
  njState.mistakes = njState.mistakes.filter(m=>m.id!==id);
  njSave(); CourseRuntime.switchView('neetjee');
}
...
function renderWeightage(){
  ...
  if(CourseSelectorShared.activeCourse === 'jee' && njState.exam !== 'jee') njState.exam = 'jee';
  else if(CourseSelectorShared.activeCourse === 'neet' && njState.exam !== 'neet') njState.exam = 'neet';
  ...
  return `<div>
    ...
    ${(!CourseSelectorShared.activeCourse || CourseSelectorShared.activeCourse==='nfsu') ? `<div class="exam-tabs">
...
export { addMistake, deleteMistake, fillOMR, omrScore, renderMistakes, renderOMR, renderRankPredictor, renderWeightage };

if (typeof window !== "undefined") {
  Object.assign(window, { addMistake, deleteMistake, fillOMR, omrScore, renderMistakes, renderOMR, renderRankPredictor, renderWeightage });
}
