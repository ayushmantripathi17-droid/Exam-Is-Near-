...
import { genId, showToast, today } from "../../../utils/helpers.js";
import { njState } from "../../../shared/exam-hub-state.js";
import { pomCleanupFS } from "./pomodoro-full.js";
import { loadAdminMaterials } from "./files-materials.js";
import { S } from "../../../core/firebase-sync.js";
import { selectedRingtone, spawnStars } from "./clock-alarms-timer.js";
...
// ── Editable exam dates ──
CourseRuntime._examDateOverrides = JSON.parse(localStorage.getItem('st_examDates')||'{}');
function getExamDate(sid){
  ...
  if(CourseRuntime._examDateOverrides[sid]) return CourseRuntime._examDateOverrides[sid];
...
function setExamDate(sid, val){
  CourseRuntime._examDateOverrides[sid] = val;
  localStorage.setItem('st_examDates', JSON.stringify(CourseRuntime._examDateOverrides));
  render();
...
function switchView(v, pushState=true){
  ...
  if(v === 'files' && AppStateShared.db && AppStateShared.currentUser) {
  ...
  if(njPill){
    if(CourseSelectorShared.activeCourse === 'nfsu' || CourseSelectorShared.activeCourse === 'nfsu1' || CourseSelectorShared.activeCourse === 'nfsu3' || CourseSelectorShared.activeCourse === 'cbse10' || CourseSelectorShared.activeCourse === 'cbse12' || CourseSelectorShared.activeCourse === null){
      njPill.style.display = 'none';
    } else {
      njPill.style.display = '';
      njPill.textContent = CourseSelectorShared.activeCourse === 'jee' ? '⚡ JEE' : '🩺 NEET';
    }
...
function setHours(h){state.hoursToday=parseFloat(h);var _hk=CourseSelectorShared.activeCourse?"hoursToday_"+CourseSelectorShared.activeCourse:"hoursToday";S(_hk,h);const el=document.getElementById("hours-display");if(el)el.textContent=state.hoursToday+"h";}
...
function logToday(){
  ...
  var _lk = CourseSelectorShared.activeCourse ? "studyLog_"+CourseSelectorShared.activeCourse : "studyLog";
...
function setTimerMode(mode){
  ...
  clearInterval(AppStateShared.timerInterval);AppStateShared.timerInterval=null;state.timerRunning=false;
...
export { _EXAM_MONTHS, _examDateToISO, _isoToExamDate, _pageMeta, _updatePageMeta, addMaterial, addPresetAlarms, cancelEdit, deleteAlarm, deleteMaterial, getDaysLeft, getExamDate, getStreak, getSubjectPct, getTotalHours, getTotalPct, hideAddForm, logToday, saveEdit, saveNewAlarm, saveSubjectNote, searchMats, setActiveSubject, setExamDate, setHours, setMatFilter, setMood, setTimerMode, showAddForm, startEdit, switchView, toggleAlarm, togglePin, toggleTopic, updateNewMat };

if (typeof window !== "undefined") {
  Object.assign(window, { _EXAM_MONTHS, _examDateToISO, _isoToExamDate, _pageMeta, _updatePageMeta, addMaterial, addPresetAlarms, cancelEdit, deleteAlarm, deleteMaterial, getDaysLeft, getExamDate, getStreak, getSubjectPct, getTotalHours, getTotalPct, hideAddForm, logToday, saveEdit, saveNewAlarm, saveSubjectNote, searchMats, setActiveSubject, setExamDate, setHours, setMatFilter, setMood, setTimerMode, showAddForm, startEdit, switchView, toggleAlarm, togglePin, toggleTopic, updateNewMat });
}
