...
import { SUBJECTS_CBSE10, SUBJECTS_JEE, SUBJECTS_NEET, SUBJECTS_NFSU, SUBJECTS_NFSU1, SUBJECTS_NFSU3, _subjectsFromIds } from "../courses/subjects-bridge.js";
import { AppStateShared, state } from "./app-state.js";
import { showToast } from "../utils/helpers.js";
import { S, pushToFirebase } from "./firebase-sync.js";

export const CourseSelectorShared = {};
// ══════════════════════════════════════════════════════════════
...
CourseSelectorShared.cbse11Stream = localStorage.getItem("cbse11Stream") || null;
...
function getCbse11Subjects(){
  ...
  // below already does.
  if(CourseSelectorShared.cbse11Stream && CBSE11_STREAMS[CourseSelectorShared.cbse11Stream]) return _subjectsFromIds(CBSE11_STREAMS[CourseSelectorShared.cbse11Stream].subjectIds);
  return [];
}

function switchCbse11Stream(streamId){
  CourseSelectorShared.cbse11Stream = streamId;
  localStorage.setItem("cbse11Stream", streamId);
  CourseSelectorShared.activeCourse = 'cbse11';
  ...
  CourseRuntime.switchView(state.view);
}

CourseSelectorShared.cbse12Stream = localStorage.getItem("cbse12Stream") || null;

function getCbse12Subjects(){
  // FIX — see getCbse11Subjects() above; same bug, same fix.
  if(CourseSelectorShared.cbse12Stream && CBSE12_STREAMS[CourseSelectorShared.cbse12Stream]) return _subjectsFromIds(CBSE12_STREAMS[CourseSelectorShared.cbse12Stream].subjectIds);
  return [];
}

function switchCbse12Stream(streamId){
  CourseSelectorShared.cbse12Stream = streamId;
  localStorage.setItem("cbse12Stream", streamId);
  CourseSelectorShared.activeCourse = 'cbse12';
...
function showCourseSelector(groupId, subId){
  ...
  if(!groupId){
    ...
    html = COURSE_TREE.map(g=>g.leaf
      ? _csBtn(`switchCourse('${g.id}')`, g.icon, g.label, g.desc, CourseSelectorShared.activeCourse===g.id)
      : _csBtn(`showCourseSelector('${g.id}')`, g.icon, g.label, g.desc, false)
    ).join('');
    html += CourseSelectorShared.activeCourse ? `<button onclick="document.getElementById('course-selector-overlay').remove()" style="margin-top:6px;width:100%;background:none;border:1px solid #1e1e1e;color:#444;padding:9px;border-radius:10px;font-family:inherit;cursor:pointer;font-size:12px">Cancel</button>` : '';
    ...
    html = group.children.map(child=>{
      if(child.leaf){
        const isActive = CourseSelectorShared.activeCourse === child.id;
    ...
    html = sub.children.map(item=>{
      const isActive = item.cbse12stream
        ? (CourseSelectorShared.activeCourse==='cbse12' && localStorage.getItem('cbse12Stream')===item.id)
        : item.cbse11stream
        ? (CourseSelectorShared.activeCourse==='cbse11' && localStorage.getItem('cbse11Stream')===item.id)
        : CourseSelectorShared.activeCourse === item.id;
...
CourseSelectorShared.activeCourse = localStorage.getItem("activeCourse") || null; // null = not selected yet

function getSubjects(){
  if(!CourseSelectorShared.activeCourse || CourseSelectorShared.activeCourse === "nfsu") return SUBJECTS_NFSU;
  if(CourseSelectorShared.activeCourse === "nfsu1") return SUBJECTS_NFSU1;
  if(CourseSelectorShared.activeCourse === "nfsu3") return SUBJECTS_NFSU3;
  if(CourseSelectorShared.activeCourse === "cbse10") return SUBJECTS_CBSE10;
  if(CourseSelectorShared.activeCourse === "cbse11") return getCbse11Subjects().length ? getCbse11Subjects() : SUBJECTS_CBSE11;
  if(CourseSelectorShared.activeCourse === "cbse12") return getCbse12Subjects().length ? getCbse12Subjects() : SUBJECTS_CBSE12;
  return COURSE_SETS[CourseSelectorShared.activeCourse]?.subjects || SUBJECTS_NFSU;
}

function switchCourse(courseId){
  CourseSelectorShared.activeCourse = courseId;
  ...
  CourseRuntime._updatePageMeta('course:'+courseId);
  ...
  if(AppStateShared.db && AppStateShared.currentUser) CourseRuntime.loadAdminMaterials().then(()=>CourseRuntime.render());
  clearTimeout(S._timer); S._timer=setTimeout(pushToFirebase,1200);
  CourseRuntime.switchView(state.view);
...
export { CBSE11_STREAMS, CBSE12_STREAMS, COURSE_SETS, COURSE_TREE, SUBJECTS_CBSE11, SUBJECTS_CBSE12, _csBreadcrumb, _csBtn, _csOverlayShell, getCbse11Subjects, getCbse12Subjects, getSubjects, renderCbse12StreamSelector, renderCourseSelector, showCourseSelector, switchCbse11Stream, switchCbse12Stream, switchCourse };

if (typeof window !== "undefined") {
  Object.assign(window, { CBSE11_STREAMS, CBSE12_STREAMS, COURSE_SETS, COURSE_TREE, SUBJECTS_CBSE11, SUBJECTS_CBSE12, _csBreadcrumb, _csBtn, _csOverlayShell, getCbse11Subjects, getCbse12Subjects, getSubjects, renderCbse12StreamSelector, renderCourseSelector, showCourseSelector, switchCbse11Stream, switchCbse12Stream, switchCourse });
}
