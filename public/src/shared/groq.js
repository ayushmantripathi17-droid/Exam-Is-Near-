...
import { CBSE12_STREAMS, CourseSelectorShared, getSubjects } from "../core/course-selector.js";
...
async function loadAIChatHistory(){
  try{
    const pro = await CourseRuntime.isProUser();
    if(!pro) return;
...
function setQuickPrompt(index){
  const p = CourseRuntime.getAIQuickPrompts()[index];
  if(!p) return;
...
function setWelcomePrompt(index){
  const c = CourseRuntime.getAIWelcomeCards()[index];
  if(!c) return;
  const input = document.getElementById('ai-input');
  if(!input) return;
...
async function sendAIMessage(){
  ...
  const _proForAI = await CourseRuntime.isProUser();
  if(!_proForAI){
  ...
  const subProgress=getSubjects().map(s=>s.name+":"+CourseRuntime.getSubjectPct(s.id)+"%").join(", ");
  const courseLabel = CourseSelectorShared.activeCourse==='neet'?'NEET UG aspirant':CourseSelectorShared.activeCourse==='jee'?'JEE (Mains & Advanced) aspirant':CourseSelectorShared.activeCourse==='nfsu'?'B.Sc. LL.B. student at NFSU':CourseSelectorShared.activeCourse==='nfsu1'?'B.Sc. LL.B. (Hons.) Sem I student at NFSU':CourseSelectorShared.activeCourse==='nfsu3'?'B.Sc. LL.B. Sem III student at NFSU':CourseSelectorShared.activeCourse==='cbse10'?'CBSE Class 10 student':CourseSelectorShared.activeCourse==='cbse12'?('CBSE Class 12 '+( CBSE12_STREAMS[CourseSelectorShared.cbse12Stream]?.label?.split('—')[1]?.trim()||'student') +' student'):'student';
  const courseExtra = CourseSelectorShared.activeCourse==='neet'?' Focus on Biology, Physics, Chemistry NCERT concepts.':CourseSelectorShared.activeCourse==='jee'?' Focus on Maths, Physics, Chemistry problem solving and formulas.':CourseSelectorShared.activeCourse==='nfsu'?' Focus on law, technology, and forensic science subjects.':CourseSelectorShared.activeCourse==='nfsu1'?' Focus on NFSU Sem I subjects: Legal Methods, Law of Tort and Consumer Protection Laws, Law and Literature, Fundamentals of Computer Organization & Embedded Systems, Basic Programming Concepts Using C, and Discrete Mathematics.':CourseSelectorShared.activeCourse==='nfsu3'?' Focus on NFSU Sem III subjects: Law of Crimes I (IPC), Constitutional Law I, Law of Contract I, Family Law I, Web Programming (HTML, JS, PHP, MySQL), and Operating System Concepts (Linux, memory, processes).':CourseSelectorShared.activeCourse==='cbse10'?' Focus on CBSE Class 10 curriculum — Maths, Science, English, Social Science.':CourseSelectorShared.activeCourse==='cbse12'?(' Focus on CBSE Class 12 board exam for '+(CBSE12_STREAMS[CourseSelectorShared.cbse12Stream]?.desc||'the selected stream')+'. Give NCERT-focused answers.'):'';
  const sysPrompt=`You are an expert AI study tutor for a ${courseLabel}. Subjects: ${subjects}. Exams start in ${CourseRuntime.getDaysLeft(CourseRuntime.getExamDate(getSubjects()[0]?.id)||'')||'a few'} days. Student progress: ${subProgress}. Hours studied: ${CourseRuntime.getTotalHours()}h. Current focus: ${ctxSubName}.${courseExtra}\nRules: Be concise yet thorough. Use **bold** for key terms. Use bullet points for lists. Use \`code\` for code snippets. Use numbered steps for procedures. Always end with a 1-line encouragement.`;
...
function renderAIChat(){
  const box=document.getElementById("ai-chat-box");
  if(!box) return;
  if(aiHistory.length===0){CourseRuntime.render();return;}
  ...
  box.innerHTML = aiHistory.map((m,i) => CourseRuntime.renderAIMsgBubble(m,i)).join("");
  // Update send button state
  const btn = document.getElementById("ai-send-btn");
  if(btn) btn.disabled = CourseRuntime.aiTyping;
  // Update dot
  const dot = document.querySelector(".ai-dot");
  if(dot){dot.className="ai-dot"+(CourseRuntime.aiTyping?" thinking":"");}
}
...
export { AI_HISTORY_KEY, AI_HISTORY_MAX, FREE_AI_MSG_LIMIT, FREE_QUIZ_LIMIT, GROQ_MODEL_FREE, GROQ_MODEL_PRO, GROQ_PROXY_URL, _freeCounterDay, _incDailyCounter, _initDailyCounter, aiHistory, askAI, freeAiMsgCount, freeQuizCount, generateQuizFromAI, loadAIChatHistory, renderAIChat, saveAIChatHistory, sendAIMessage, setQuickPrompt, setWelcomePrompt };

if (typeof window !== "undefined") {
  Object.assign(window, { AI_HISTORY_KEY, AI_HISTORY_MAX, FREE_AI_MSG_LIMIT, FREE_QUIZ_LIMIT, GROQ_MODEL_FREE, GROQ_MODEL_PRO, GROQ_PROXY_URL, _freeCounterDay, _incDailyCounter, _initDailyCounter, aiHistory, askAI, freeAiMsgCount, freeQuizCount, generateQuizFromAI, loadAIChatHistory, renderAIChat, saveAIChatHistory, sendAIMessage, setQuickPrompt, setWelcomePrompt });
}
