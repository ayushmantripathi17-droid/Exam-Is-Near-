...
async function handleFileSelect(files){
  ...
  for(const file of Array.from(files)){
    ...
    // FREE (or Pro fallback): Upload to Cloudflare R2 via the Worker
    // (replaces Firebase Storage — doesn't need the Firebase project on
    // Blaze, so it isn't affected by a GCP billing suspension)
    if(currentUser){
      ...
      try{
        ...
        const form = new FormData();
        form.append("file", file);
        form.append("fileId", fileObj.id);
        form.append("fileName", file.name);
        const res = await fetch("https://exam-is-near-groq-proxy.examisnearbyarksetu.workers.dev/uploadFile", {
          method: "POST",
          headers: { "Authorization": "Bearer " + idToken },
          body: form
        });
        const json = await res.json();
        if(!res.ok || json.error) throw new Error((json.error && json.error.message) || ("Upload failed: "+res.status));

        fileObj.downloadURL = json.result.downloadURL;
        fileObj.storage="r2";
        fileObj.synced=true;
